...
AS
    ...
    BEGIN
        ...
        DELETE FROM   SESS_JOB_DEPENDENCY_BCKP
              WHERE   PARENT_JOB_NAME = JOB_NAME AND ENGINE_ID = ENGINE_ID_IN;
    ...
    BEGIN
        ...
        V_STEP := 'Value of old LOAD_DATE is '|| to_char(V_LOAD_DATE);
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
        ...
        V_STEP := 'Value of NEW_LOAD_DATE is '|| to_char(V_NEW_LOAD_DATE);
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

        V_STEP := 'Check condition - LOAD_DATE =  V_NEW_LOAD_DATE';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

        IF V_NEW_LOAD_DATE=V_LOAD_DATE
        THEN -- new load_date will be > max_load_date
            V_STEP := 'ERROR> New load_date is the same as previous one...';
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
            RAISE EX_PROCEDURE_END;
        END IF;
    ...
    BEGIN
        ...
        V_STEP := 'select PREV_LOAD_DATE and LOAD_DATE';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
        ...
        */
        ...
        V_STEP := 'select day in week from PREV_LOAD_DATE: ' || TO_CHAR(V_PREV_LOAD_DATE, 'YYYY-MM-DD');
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

        V_DAY_IN_WEEK := PCKG_TOOLS.F_GET_DAY_OF_WEEK(V_PREV_LOAD_DATE);

        IF TO_NUMBER(TO_CHAR(V_PREV_LOAD_DATE, 'DD')) = TO_NUMBER(TO_CHAR(LAST_DAY(V_PREV_LOAD_DATE), 'DD'))
        THEN
        ...
        ELSE
          ...
          V_DAY_IN_MONTH := TO_NUMBER(TO_CHAR(V_PREV_LOAD_DATE, 'DD'));
        END IF;
        ...
        (
          ...
          (
                ...
                FROM   SESS_STATUS SS
                WHERE   ENGINE_ID = ENGINE_ID_IN
                AND LOAD_DATE = V_PREV_LOAD_DATE
      ...
      INSERT INTO STAT_STATUS(JOB_ID
        ...
        V_STEP := 'select day in week (actual load) FROM LOAD_DATE: ' || TO_CHAR(V_LOAD_DATE, 'YYYY-MM-DD');
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
        ...
        END IF;
          ...
          V_STEP := 'Inserting into SESS_JOB_STATISTICS';
          V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
          V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
        ...
        GROUP BY JOB_NAME;
          ...
          SELECT COUNT(*) INTO V_CNT FROM SESS_JOB_STATISTICS;

          V_STEP := 'Insert into SESS_JOB_STATISTICS inserted '  || TO_CHAR(V_CNT) || ' records';
          V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
          V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
