  ...
  IS
    ...
    *******************************************************************************/
    ...
    BEGIN
      ...
      BEGIN
        V_STEP := 'Check valid root vertex';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
    ...
    ------------------------------------------------------------------------*/
      ...
      BEGIN
        ...
        V_STEP := 'Initialize temporary structires';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
      ...
      BEGIN
        V_STEP := 'Start computation of maximal distances to all vertices';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
      ...
      BEGIN
        V_STEP := 'Compute count of edges that forms a critical path';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
        ...
        END LOOP;
        V_STEP := 'Derive order and distances of vertices in critical path';
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
      ...
      EXCEPTION
        WHEN GRAPH_NOT_FOUND
          THEN
            ...
            ERRCODE_OUT := SQLCODE;
            ERRLINE_OUT := SUBSTR('THE TABLE TEMP_CRTP_DEPENDENCY_GRAPH WAS NOT FOUND ' || V_STEP, 1, 1024);
    ...
    BEGIN
		...
		IF V_ACTUAL_LOAD_DATE = LOAD_DATE_IN THEN
			...
			SELECT 	SB.JOB_ID,
				  ...
				  TJS.LOAD_DATE,
				  TJS.ENGINE_ID
			FROM TEMP_CRTP_JOB_STATISTICS TJS
      INNER JOIN SESS_JOB_DEPENDENCY_BCKP SB ON SB.JOB_NAME=TJS.JOB_NAME
