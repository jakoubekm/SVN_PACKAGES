...
IS
    ...
    BEGIN
        ...
        LOOP
            ...
            SELECT   TO_CHAR(CTRLPAR1.PARAM_VAL_DATE, 'DD.MM.YYYY')
              INTO   V_LOAD_DATE
            ...
            THEN
                ...
                PCKG_FWRK.SP_SAVE_STAT_LOG_EVENT_HIST(
                  ...
                  , NOTE_IN                     =>   'Job '
                                                  ...
                                                  || ' job_id='
                                                  || R2.JOB_ID
                                                  || ' has not finished as it is expected. Running duration is='
    ...
    IS
        ...
        *******************************************************************************/
        ...
        IS
              --vyber novy zaznam
              SELECT   COUNT(SLEH.JOB_ID) POCET
                     , SLEH.JOB_NAME
                     , SLEH.JOB_ID
                     , SLEH.DWH_DATE
                     , SLEH.ERROR_CD
               ...
               WHERE   SLEH.SENT_TS IS NULL
                   AND SLEH.JOB_ID IS NOT NULL
            --                 AND TO_CHAR(SLEH.DWH_DATE, 'DD.MM.YYYY') = TO_CHAR(V_DWH_DATE, 'DD.MM.YYYY')
            GROUP BY   SLEH.JOB_NAME
                     , SLEH.JOB_ID
                     , SLEH.DWH_DATE
                     , SLEH.ERROR_CD
              HAVING   COUNT(SLEH.JOB_ID) >= 1;

        CURSOR CUR_GET_ALERT_ROW_HIST_NR(XJOB_ID VARCHAR2, XDWH_DATE DATE, XERROR_CD VARCHAR2)
        IS
            --zjisti, jestli novemu zaznamu predchazely historicke zaznamy
            SELECT   COUNT(SLEH.JOB_ID) POCET
              FROM   STAT_LOG_EVENT_HIST SLEH
             WHERE   SLEH.SENT_TS IS NOT NULL
                 ...
                 AND NVL(SLEH.ERROR_CD, 1) = NVL(XERROR_CD, 1) /*            GROUP BY   SLEH.JOB_NAME, SLEH.DWH_DATE
                                                                             HAVING   COUNT(SLEH.JOB_NAME) >= 1*/
                                                              ;

        CURSOR CUR_GET_ALERT_ROW_HIST(XJOB_ID VARCHAR2, XDWH_DATE DATE, XERROR_CD VARCHAR2)
        IS
              --zjisti, jestli novemu zaznamu predchazely historicke zaznamy
              SELECT   COUNT(SLEH.SENT_TS) POCET
                     , SLEH.JOB_NAME
                     , SLEH.DWH_DATE
            ...
            GROUP BY   SLEH.JOB_NAME,SLEH.DWH_DATE;

        CURSOR CUR_GET_ALERT_ROW_DETAIL(XJOB_ID VARCHAR2, XDWH_DATE DATE, XERROR_CD VARCHAR2)
        IS
            --vybereme detaily k novemu zaznamu
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR R5 IN CUR_GET_ALERT_ROW_HIST_NR(R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD)
            LOOP
                V_STEP := 'jisti, jestli se nove zaznamy opakuji';
                V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
                V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

                IF R5.POCET = 0
               AND V_GET_ERROR_CD_NAME_SHORT NOT IN ('SCH', 'INIT')
                THEN
                    ...
                    FOR R4 IN CUR_GET_ALERT_ROW_DETAIL(R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD)
                    LOOP
                        ...
                        UPDATE   STAT_LOG_EVENT_HIST SLEH
                         ...
                         WHERE   SLEH.SENT_TS IS NULL
                             ...
                             AND SLEH.ERROR_CD = R1.ERROR_CD;
                    END LOOP;
                ELSIF R5.POCET = 0
                  AND V_GET_ERROR_CD_NAME_SHORT IN ('SCH', 'INIT')
                THEN
                    UPDATE   STAT_LOG_EVENT_HIST SLEH
                       SET   SLEH.SENT_TS = CURRENT_TIMESTAMP
                     WHERE   SLEH.SENT_TS IS NULL
                         ...
                         AND SLEH.ERROR_CD = R1.ERROR_CD;
                ELSE
                    FOR R2 IN CUR_GET_ALERT_ROW_HIST(R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD)
                    LOOP
                        FOR R3 IN CUR_GET_ALERT_ROW_DETAIL(R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD)
                        LOOP
                            V_STEP :=
                            ...
                            IF R2.POCET BETWEEN 1 AND V_MESSAGE_REITERATION_COUNT
                            ...
                            THEN
                                ...
                                UPDATE   STAT_LOG_EVENT_HIST SLEH
                                 ...
                                 WHERE   SLEH.SENT_TS IS NULL
                                     ...
                                     AND SLEH.DWH_DATE = R1.DWH_DATE
                                     AND SLEH.ERROR_CD = R1.ERROR_CD;
                            ELSIF R5.POCET >= V_SCHEDULER_OFF_COUNT
                              AND PCKG_TOOLS.F_SEC_BETWEEN(R2.MIN_SENT_TS, CURRENT_TIMESTAMP) <= V_MESSAGE_REITERATION_INTERVAL
                              AND V_GET_ERROR_CD_NAME_SHORT = 'SCH'
                            THEN
                                V_STEP := 'pokud je chyba scheduler a v povolenem intervalu potom zapis do msg';
                                V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
                                V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
                                PCKG_FWRK.SP_SAVE_STAT_LOG_MESSAGE_HIST(LOG_EVENT_ID_IN           => R3.LOG_EVENT_ID
                                                                      , ERROR_CD_IN               => R3.ERROR_CD
                                ...
                                UPDATE   STAT_LOG_EVENT_HIST SLEH
                                 ...
                                 WHERE   SLEH.SENT_TS IS NULL
                                     ...
                                     AND SLEH.DWH_DATE = R1.DWH_DATE
                                     AND SLEH.ERROR_CD = R1.ERROR_CD;
                            ELSIF R5.POCET >= V_INITIALIZATION_PROBLEM_COUNT
                              AND PCKG_TOOLS.F_SEC_BETWEEN(R2.MIN_SENT_TS, CURRENT_TIMESTAMP) <= V_MESSAGE_REITERATION_INTERVAL
                              AND V_GET_ERROR_CD_NAME_SHORT = 'INIT'
                            THEN
                                V_STEP := 'pokud je chyba initialization a v povolenem intervalu potom zapis do msg';
                                V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
                                V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
                                PCKG_FWRK.SP_SAVE_STAT_LOG_MESSAGE_HIST(LOG_EVENT_ID_IN           => R3.LOG_EVENT_ID
                                                                      , ERROR_CD_IN               => R3.ERROR_CD
