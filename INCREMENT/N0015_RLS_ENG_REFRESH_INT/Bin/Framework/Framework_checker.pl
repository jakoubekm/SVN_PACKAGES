...
$OID_Parse_Suffix="1.3.6.1.4.1.191.103.";
$IP_receiver="TCP:" . $ENV{'SNMP_DEST'};  # adresa SNMP receiveru
$PARAM="-m ALL -Ci -v 2c -c public ".$IP_receiver." \"\" ".$OID_Parse_Suffix."0.1 "; ### Ci je INFORM
...
sub run_SQL_procedure {
	...
	$SQL = <<SQLEND;
BEGIN
	PCKG_FWRK.$procedure(:debug_in, :exit_cd, :errmsg_out, :errcode_out, :errline_out);
END;
SQLEND
...
sub sent_SNMP {
	...
	for $log_event_id (sort keys %$record) {
		...
		$Param=$PARAM;
		$Param=$Param.$OID_Parse_Suffix."2.1 s \"" . $Log_Event_ID . "\" ";
		$Param=$Param.$OID_Parse_Suffix."2.2 s \"" . $Error_CD."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.3 s \"" . $Job_Name."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.4 s \"" . $Job_ID."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.5 s \"" . $Severity."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.6 s \"" . $Notification_Type_CD."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.7 s \"" . $Event_Ds."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.8 s \"" . $Recommendation_Ds."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.9 s \"" . $Note."\" ";
		$Param=$Param.$OID_Parse_Suffix."2.10 s \"" . $Detected_TS . "\" ";
		$Param=$Param.$OID_Parse_Suffix."2.11 s \"" . $Engine_Name . "\" ";
		$Param=$Param.$OID_Parse_Suffix."2.12 s \"" . $System_Name . "\" ";
...
connect_Oracle;
$exit_cd = 0;
($exit_cd == 0) and $exit_cd = run_SQL_procedure ("SP_FWRK_CHECK_SOURCE_DELIVERY");
($exit_cd == 0) and $exit_cd = run_SQL_procedure ("SP_FWRK_CHECK_WD_STATUS");
($exit_cd == 0) and $exit_cd = run_SQL_procedure ("SP_FWRK_CHECK_SCHED_STATUS");
($exit_cd == 0) and $exit_cd = run_SQL_procedure ("SP_FWRK_CHECK_INITIALIZATION");
($exit_cd == 0) and $exit_cd = run_SQL_procedure ("SP_FWRK_CHECK_NOTIFICATION");
($exit_cd == 0) and $exit_cd = run_SQL_procedure ("SP_FWRK_MESSAGE_GEN");
get_Data;
sent_SNMP;
write_Result;
