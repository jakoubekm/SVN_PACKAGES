...
IS
            ...
            THEN
                ...
                THEN
                    ...
                    PCKG_FWRK.SP_SAVE_STAT_LOG_EVENT_HIST(EVENT_TS_IN                 => CURRENT_TIMESTAMP
                                                        ...
                                                        , JOB_ID_IN                   => -3
                                                        , SEVERITY_LEVEL_CD_IN        => NVL(PCKG_FWRK.F_GET_SEVERITY_LEVEL_CD('ENGINE'),8)
                                                        , ERROR_CD_IN                 => 'ENGINE'
                                                        , EVENT_CD_IN                 => NULL
                                                        , EVENT_DS_IN                 => NULL
                                                        , START_TS_IN                 => NULL
    ...
    IS
        ...
        *******************************************************************************/
        ...
        V_ENGINE_PROBLEM_COUNT   INTEGER := PCKG_FWRK.F_GET_CTRL_PARAMETERS('ENGINE_PROBLEM_COUNT', 'param_val_int');
        ...
        CURSOR CUR_GET_ALERT_ROW_HIST_NR(XJOB_NAME VARCHAR2, XJOB_ID NUMBER, XDWH_DATE DATE, XERROR_CD VARCHAR2, XN_RUN NUMBER, XSYSTEM_NAME VARCHAR)
        IS
            ...
            SELECT   COUNT(DISTINCT SLEH.JOB_ID) POCET
             ...
             WHERE   SLEH.SENT_TS IS NOT NULL
                 ...
                 AND NVL(SLEH.SYSTEM_NAME, 1) = NVL(XSYSTEM_NAME, 1);

        CURSOR CUR_GET_ALERT_ROW_HIST(XJOB_NAME VARCHAR2, XJOB_ID VARCHAR2, XDWH_DATE DATE, XERROR_CD VARCHAR2, XN_RUN NUMBER, XSYSTEM_NAME VARCHAR)
        IS
              ...
              SELECT   COUNT(DISTINCT SLEH.SENT_TS) POCET
               ...
               WHERE   SLEH.SENT_TS IS NOT NULL
                   ...
                   AND NVL(SLEH.N_RUN, 1) = NVL(XN_RUN, 1)
                   AND NVL(SLEH.SYSTEM_NAME, 1) = NVL(XSYSTEM_NAME, 1)
            GROUP BY   SLEH.JOB_NAME,SLEH.JOB_ID,SLEH.DWH_DATE,SLEH.N_RUN, SLEH.SYSTEM_NAME;

        CURSOR CUR_GET_ALERT_ROW_DETAIL(XJOB_NAME VARCHAR2, XJOB_ID VARCHAR2, XDWH_DATE DATE, XERROR_CD VARCHAR2, XN_RUN NUMBER, XSYSTEM_NAME VARCHAR)
        IS
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR R5 IN CUR_GET_ALERT_ROW_HIST_NR(R1.JOB_NAME, R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD, R1.N_RUN, R1.SYSTEM_NAME)
            LOOP
              V_STEP := 'zjisti, jestli se nove zaznamy opakuji';
              V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
              V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

              IF R5.POCET = 0 AND V_GET_ERROR_CD_NAME_SHORT NOT IN ('SCH', 'INIT', 'ENG')
              THEN
                    ...
                    FOR R4 IN CUR_GET_ALERT_ROW_DETAIL(R1.JOB_NAME, R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD, R1.N_RUN, R1.SYSTEM_NAME)
                    LOOP
                        ...
                        UPDATE   STAT_LOG_EVENT_HIST SLEH
                         ...
                         WHERE   SLEH.SENT_TS IS NULL
                             ...
                             AND NVL(SLEH.SYSTEM_NAME, 1) = NVL(R1.SYSTEM_NAME, 1);
                    END LOOP;

              ELSIF ((R1.POCET_NOT_SEND >= V_SCHEDULER_OFF_COUNT AND V_GET_ERROR_CD_NAME_SHORT = 'SCH')
                    OR (R1.POCET_NOT_SEND  >= V_INITIALIZATION_PROBLEM_COUNT AND V_GET_ERROR_CD_NAME_SHORT = 'INIT')
                    OR (R1.POCET_NOT_SEND  >= V_ENGINE_PROBLEM_COUNT AND V_GET_ERROR_CD_NAME_SHORT = 'ENG'))
                THEN
                  IF R5.POCET = 0
                    THEN
                      FOR R3 IN CUR_GET_ALERT_ROW_DETAIL(R1.JOB_NAME, R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD, R1.N_RUN, R1.SYSTEM_NAME)
                      LOOP
                      ...
                      END LOOP;
                  ELSE
                    FOR R2 IN CUR_GET_ALERT_ROW_HIST(R1.JOB_NAME, R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD, R1.N_RUN, R1.SYSTEM_NAME)
                    LOOP
                      IF PCKG_TOOLS.F_SEC_BETWEEN(R2.MAX_SENT_TS, CURRENT_TIMESTAMP) >= V_MESSAGE_REITERATION_INTERVAL
                      THEN
                        FOR R3 IN CUR_GET_ALERT_ROW_DETAIL(R1.JOB_NAME, R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD, R1.N_RUN, R1.SYSTEM_NAME)
                        LOOP
                          V_STEP := 'pokud je chyba scheduler, presahne pocet iteraci, pak rozesila v intervalu';
                  ...
                  END IF;
              ELSE
                IF V_GET_ERROR_CD_NAME_SHORT NOT IN ('SCH', 'INIT', 'ENG') THEN
                  FOR R2 IN CUR_GET_ALERT_ROW_HIST(R1.JOB_NAME, R1.JOB_ID, R1.DWH_DATE, R1.ERROR_CD, R1.N_RUN, R1.SYSTEM_NAME)
                  LOOP
    ...
    PROCEDURE SP_FWRK_CHECK_SOURCE_DELIVERY (
      ...
      *******************************************************************************/
      ...
      V_CURRENT_MINUTE_SNIFFER_JOB    INTEGER;
      V_DELIVERED                   INTEGER;
      V_SKIPPED                     INTEGER;
      ...
      FOR R_CURR_SOURCE IN SOURCE_TO_NOTIFY
        LOOP  -- LOOP SOURCE
         ...
         DECLARE
            ...
            IS
                 SELECT RUNPLAN,
                        (CASE WHEN ALERT_TM_IN_MINUTES=0 THEN  ALERT_ZONE_START_TM*60 ELSE ALERT_ZONE_START_TM END) AS ALERT_ZONE_START_TM,
         ...
         THEN
            ...
            SELECT     EXTRACT (DAY FROM (  R_CURR_SOURCE.SNIFFER_LAST_RUN - V_INIT_END_TS))* 24 * 60
                   + EXTRACT (HOUR FROM (  R_CURR_SOURCE.SNIFFER_LAST_RUN - V_INIT_END_TS))* 60
                   + EXTRACT (MINUTE FROM (  R_CURR_SOURCE.SNIFFER_LAST_RUN - V_INIT_END_TS))
            INTO V_CURRENT_MINUTE_SNIFFER_JOB
            FROM DUAL;
         ELSE
            --V_CURRENT_HOUR--
            SELECT     EXTRACT (DAY FROM (  CURRENT_TIMESTAMP - R_CURR_SOURCE.SNIFFER_LOAD_DATE))* 24 * 60
                   + EXTRACT (HOUR FROM (  CURRENT_TIMESTAMP - R_CURR_SOURCE.SNIFFER_LOAD_DATE))* 60
                   + EXTRACT (MINUTE FROM (  CURRENT_TIMESTAMP - R_CURR_SOURCE.SNIFFER_LOAD_DATE))
            INTO V_CURRENT_MINUTE
            FROM DUAL;

            --V_CURRENT_HOUR_SNIFFER_JOB--
            SELECT     EXTRACT ( DAY FROM (  R_CURR_SOURCE.SNIFFER_LAST_RUN - R_CURR_SOURCE.SNIFFER_LOAD_DATE))* 24 * 60
                   + EXTRACT ( HOUR FROM (  R_CURR_SOURCE.SNIFFER_LAST_RUN - R_CURR_SOURCE.SNIFFER_LOAD_DATE))* 60
                   + EXTRACT ( MINUTE FROM (  R_CURR_SOURCE.SNIFFER_LAST_RUN - R_CURR_SOURCE.SNIFFER_LOAD_DATE))
              INTO V_CURRENT_MINUTE_SNIFFER_JOB
              FROM DUAL;
         END IF;

         --if alert zone is broken and sniffer was runnig after break and V_ALERT_ZONE_START_TM<>V_ALERT_ZONE_END_TM
         IF (V_CURRENT_MINUTE >= V_ALERT_ZONE_START_TM
             AND V_CURRENT_MINUTE_SNIFFER_JOB >= V_ALERT_ZONE_START_TM AND V_ALERT_ZONE_START_TM<>V_ALERT_ZONE_END_TM)
         THEN
