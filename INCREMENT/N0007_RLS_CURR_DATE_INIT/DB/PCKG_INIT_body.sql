        ...
        *******************************************************************************/
        ...
        IS
            SELECT   CJ.JOB_NAME
              ...
              FROM   CTRL_JOB CJ
             WHERE   CJ.ENGINE_ID = ENGINE_ID_IN
                 AND CJ.JOB_NAME IN (  SELECT   SS.JOB_NAME
                                         FROM   SESS_STATUS SS
                                     GROUP BY   SS.JOB_NAME);
        ...
        E_SP_INIT_PLAN_FAILED EXCEPTION;
        V_AVG_FIRST_START_TS   SESS_JOB_STATISTICS.FIRST_START_TS%TYPE;
        V_AVG_LAST_START_TS    SESS_JOB_STATISTICS.LAST_START_TS%TYPE;
        V_AVG_LAST_STATUS_TS   SESS_JOB_STATISTICS.LAST_STATUS_TS%TYPE;
        V_MAX_END_TS           SESS_JOB_STATISTICS.END_TS%TYPE;
        V_AVG_AVG_DURATION     SESS_JOB_STATISTICS.AVG_DURATION%TYPE;
        V_AVG_AVG_END_TM       SESS_JOB_STATISTICS.AVG_END_TM%TYPE;
        V_IGNORE_STAT          SESS_JOB_STATISTICS.IGNORE_STAT%TYPE;
    BEGIN
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        V_ALL_DBG_INFO(V_DBG_INFO_ID) := 'Execute ' || C_PCKG_NAME || '.' || C_PROC_NAME;
        V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
        ...
        */
        SELECT   MIN(SS.LOAD_DATE) LOAD_DATE
              INTO   V_LOAD_DATE
        FROM   SESS_STATUS SS
        ...
        SELECT V_LOAD_DATE -
        (SELECT   PARAM_VAL_INT / 1440 -- number of minutes per day
              FROM   CTRL_PARAMETERS
              WHERE   PARAM_NAME = 'INITIALIZATION_RETENTION_PERIOD'
              AND PARAM_CD = ENGINE_ID_IN)
        INTO V_PREV_LOAD_DATE
        FROM DUAL;
        ...
        FOR R1 IN GET_CTRL_JOB_DETAILS
        LOOP
            EXIT WHEN GET_CTRL_JOB_DETAILS%NOTFOUND;
            V_STEP := 'select first_start_ts, last_start_ts';
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

            --select first_start_ts, last_start_ts
            SELECT   MIN(SS.STATUS_TS), MAX(SS.STATUS_TS) FIRST_START_TS
              INTO   V_FIRST_START_TS, V_LAST_START_TS
              FROM   SESS_STATUS SS
             WHERE   SS.ENGINE_ID = ENGINE_ID_IN
                 AND SS.JOB_NAME = R1.JOB_NAME
                 AND SS.LOAD_DATE = V_LOAD_DATE
                 AND PCKG_TOOLS.F_GET_IGNORE_STATS(SS.APPLICATION_ID) = 0
                 AND SS.STATUS IN (SELECT   CJS.STATUS
                                     FROM   CTRL_JOB_STATUS CJS
                                    WHERE   CJS.RUNABLE = 'RUNNING');

            V_STEP := 'select last_status_ts, n_run';
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

            --select last_status_ts,n_run
            SELECT   MAX(SS.STATUS_TS) LAST_STATUS_TS, MAX(SS.N_RUN) N_RUN
              INTO   V_LAST_STATUS_TS, V_N_RUN
              FROM   SESS_STATUS SS
             WHERE   SS.ENGINE_ID = ENGINE_ID_IN
                 AND SS.JOB_NAME = R1.JOB_NAME
                 AND SS.LOAD_DATE = V_LOAD_DATE
            ...
            V_STEP := 'select end_ts';
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

            --select end_ts
            SELECT   MAX(SS.STATUS_TS) END_TS
              INTO   V_END_TS
              FROM   SESS_STATUS SS
             WHERE   SS.ENGINE_ID = ENGINE_ID_IN
                 AND SS.JOB_NAME = R1.JOB_NAME
            ...
            V_STEP := 'select last_status SS.ENGINE_ID=' || ENGINE_ID_IN || 'AND SS.JOB_NAME=' || R1.JOB_NAME || 'AND SS.STATUS_TS=' || V_LAST_STATUS_TS;
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;

            -- select last_status
            SELECT   MAX(SS.STATUS) LAST_STATUS
              INTO   V_LAST_STATUS
              FROM   SESS_STATUS SS, CTRL_JOB_STATUS CJS
             WHERE   SS.STATUS = CJS.STATUS
                 AND CJS.FINISHED = 1
                 AND SS.ENGINE_ID = ENGINE_ID_IN
                 AND SS.JOB_NAME = R1.JOB_NAME
                 AND SS.LOAD_DATE = V_LOAD_DATE
                 AND SS.STATUS_TS = NVL(V_LAST_STATUS_TS, SS.STATUS_TS)
                 AND PCKG_TOOLS.F_GET_IGNORE_STATS(SS.APPLICATION_ID) = 0;

            V_STEP := 'select avg duration';
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
            V_AVG_DURATION := PCKG_TOOLS.F_SEC_BETWEEN(V_LAST_START_TS, V_END_TS);

            V_STEP := 'select avg_end_tm';
            V_DBG_INFO_ID := V_DBG_INFO_ID + 1;
            V_ALL_DBG_INFO(V_DBG_INFO_ID) := ' STEP> ' || V_STEP;
            V_AVG_END_TM := PCKG_TOOLS.F_SEC_BETWEEN(V_PREV_LOAD_DATE, V_END_TS);
