  ...
  CREATE OR REPLACE PACKAGE BODY "PCKG_GUI"
AS
   ...
   PROCEDURE SP_GUI_HEADER_SYS_STAT(SYS_STATUS_OUT   OUT REF_GUI_HEADER_SYS_STATUS
    ...
    IS
        ...
        *******************************************************************************/
        ...
        V_ALL_DBG_INFO             PCKG_PLOG.T_VARCHAR2;
        V_DBG_INFO_ID              INTEGER := 0;
        V_SYSTEM_NAME              Ctrl_Parameters.Param_Val_Char%type;
        V_STATE                    VARCHAR2(3);
    ...
    BEGIN
        ...
        OPEN LC_GUI_HEADER_SYS_STATUS FOR
            SELECT   A.PARAM_VAL_CHAR,
              CASE WHEN B.PARAM_VAL_INT IS NOT NULL THEN
                CASE WHEN PCKG_ENGINE.F_ENG_CHECK_WD_SYS_STATUS(A.PARAM_CD,A.PARAM_VAL_CHAR)+ B.PARAM_VAL_INT = 0 THEN 'ON' ELSE 'OFF' END
              ELSE
                CASE WHEN PCKG_ENGINE.F_ENG_CHECK_WD_SYS_STATUS(A.PARAM_CD,A.PARAM_VAL_CHAR) = 0 THEN 'ON' ELSE 'OFF' END
              END SYS_STATUS
              FROM   CTRL_PARAMETERS A
              LEFT JOIN (SELECT PARAM_VAL_CHAR, PARAM_VAL_INT FROM CTRL_PARAMETERS WHERE PARAM_NAME='SYSTEM_OFF') B ON B.PARAM_VAL_CHAR=A.PARAM_VAL_CHAR
        ...
        FETCH LC_GUI_HEADER_SYS_STATUS INTO v_SYSTEM_NAME, V_STATE;
        WHILE LC_GUI_HEADER_SYS_STATUS%FOUND LOOP
          IF V_STATE = 'ON' THEN
            SYS_NUMBER_ON := SYS_NUMBER_ON+1;
          ELSE
            SYS_NUMBER_OFF := SYS_NUMBER_OFF+1;
          END IF;
          FETCH LC_GUI_HEADER_SYS_STATUS INTO v_SYSTEM_NAME, V_STATE;
        END LOOP;

        --REOPEN CURSOR
        CLOSE LC_GUI_HEADER_SYS_STATUS;


        OPEN LC_GUI_HEADER_SYS_STATUS FOR
          SELECT   A.PARAM_VAL_CHAR,
              CASE WHEN B.PARAM_VAL_INT IS NOT NULL THEN
                CASE WHEN B.PARAM_VAL_INT = 1 THEN 'DISABLED'
                  WHEN PCKG_ENGINE.F_ENG_CHECK_WD_SYS_STATUS(A.PARAM_CD,A.PARAM_VAL_CHAR) = 0 THEN 'ON'
                ELSE 'OFF' END
              ELSE
                CASE WHEN PCKG_ENGINE.F_ENG_CHECK_WD_SYS_STATUS(A.PARAM_CD,A.PARAM_VAL_CHAR) = 0 THEN 'ON' ELSE 'OFF' END
              END SYS_STATUS
...
PROCEDURE SP_GUI_VIEW_CTRL_SCHED_NUM_JOB(ENG_ID_IN IN  INTEGER
    ...
    PROCEDURE SP_GUI_SYSTEM_ENABLE(ENG_ID_IN IN  INTEGER
                                          , USER_IN IN    VARCHAR2
                                          , DEBUG_IN IN   INTEGER:= 0
                                          , EXIT_CD_OUT   OUT NOCOPY NUMBER
                                          , ERRMSG_OUT   OUT NOCOPY VARCHAR2
    ...
    IS
        ...
        *******************************************************************************/
        --constants
        C_PROC_NAME         CONSTANT VARCHAR2(64) := 'SP_GUI_SYSTEM_ENABLE';
        C_PROC_VERSION      CONSTANT VARCHAR2(16) := '1.0';
        -- local variables
        V_STEP              VARCHAR2(1024);
        V_ALL_DBG_INFO      PCKG_PLOG.T_VARCHAR2;
    ...
    BEGIN
        ...
        PCKG_GUI.SP_GUI_SET_LOG_CTRL_ACTION(
            USER_NAME_IN     => USER_IN
          , ACTION_IN        => 'enablesystem'
          , SQL_CODE_IN      => 'UPDATE CTRL_PARAMETERS SET param_val_int = ' || 0 || ' WHERE PARAM_NAME = ''MAX_CONCURRENT_JOBS'' AND param_val_char = ''' || system_name_in || ''''
          , V_ENGINE_ID_IN   => ENG_ID_IN
          , DEBUG_IN         => DEBUG_IN
          , EXIT_CD          => EXIT_CD_OUT
          , ERRMSG_OUT       => ERRMSG_OUT
    ...
    EXCEPTION
        ...
        THEN
            ...
            PCKG_PLOG.FATAL();
            PCKG_PLOG.SETPROCPARAMS(PROCEDURE_NAME_IN => C_PROC_NAME, ALL_ARGUMENTS_IN => V_ALL_DBG_INFO);
    END SP_GUI_SYSTEM_ENABLE;

    PROCEDURE SP_GUI_SYSTEM_DISABLE(ENG_ID_IN IN  INTEGER
                                          , USER_IN IN    VARCHAR2
                                          , DEBUG_IN IN   INTEGER:= 0
                                          , EXIT_CD_OUT   OUT NOCOPY NUMBER
                                          , ERRMSG_OUT   OUT NOCOPY VARCHAR2
    ...
    IS
        ...
        *******************************************************************************/
        --constants
        C_PROC_NAME         CONSTANT VARCHAR2(64) := 'SP_GUI_SYSTEM_DISABLE';
        C_PROC_VERSION      CONSTANT VARCHAR2(16) := '1.0';
        -- local variables
        V_STEP              VARCHAR2(1024);
        V_ALL_DBG_INFO      PCKG_PLOG.T_VARCHAR2;
    ...
    BEGIN
        ...
        PCKG_GUI.SP_GUI_SET_LOG_CTRL_ACTION(
            USER_NAME_IN     => USER_IN
          , ACTION_IN        => 'disablesystem'
          , SQL_CODE_IN      => 'UPDATE CTRL_PARAMETERS SET param_val_int = ' || 1 || ' WHERE PARAM_NAME = ''MAX_CONCURRENT_JOBS'' AND param_val_char = ''' || system_name_in || ''''
          , V_ENGINE_ID_IN   => ENG_ID_IN
          , DEBUG_IN         => DEBUG_IN
          , EXIT_CD          => EXIT_CD_OUT
          , ERRMSG_OUT       => ERRMSG_OUT
