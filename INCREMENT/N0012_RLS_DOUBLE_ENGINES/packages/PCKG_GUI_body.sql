...
/*
    ...
    PROCEDURE SP_GUI_JOB_BLOCK_EXEC(JOB_ID_IN IN  NUMBER
                                  , DEBUG_IN IN   INTEGER:= 0
    ...
    IS
        ...
        Date:    2011-10-05
        -------------------------------------------------------------------------------
        Description:The purpose of this stored procedure is execution of set status BLOCKED with jobs.
        -------------------------------------------------------------------------------
        Modified:
        Version:
        Date:
        ...
        THEN
            ...
            ELSE -- Job can't be blocked, block child jobs
                ...
                BEGIN -- Level_2
                    ...
                    LOOP
                        SP_GUI_JOB_BLOCK_EXEC(JOB_ID_IN     => V_NEXT_JOB_ID.JOB_ID
                                            , DEBUG_IN      => DEBUG_IN
                                            , EXIT_CD       => EXIT_CD
                                            , ERRMSG_OUT    => ERRMSG_OUT
                                            , ERRCODE_OUT   => ERRCODE_OUT
    ...
    BEGIN
        ...
        -- If job can be blocked, do it
        IF UPPER(V_RUNABLE) NOT IN ('RUNNING', 'FAILED', 'FINISHED')
        THEN
            PCKG_ENGINE.SP_ENG_UPDATE_STATUS(JOB_ID_IN           => JOB_ID_IN
                                           , LAUNCH_IN           => 0
                                           , SIGNAL_IN           => 'BLOCK'
        ...
        ELSE -- Job can't be blocked, block child jobs
            ...
            BEGIN -- Level_2
                FOR V_NEXT_JOB_ID IN JOB_CUR
                LOOP
                    PCKG_ENGINE.SP_ENG_UPDATE_STATUS(JOB_ID_IN           => V_NEXT_JOB_ID.JOB_ID
                                                   , LAUNCH_IN           => 0
                                                   , SIGNAL_IN           => 'BLOCK'
                                                   , REQUEST_IN          => 'BLOCK'
                                                   , ENGINE_ID_IN        => ENG_ID_IN
